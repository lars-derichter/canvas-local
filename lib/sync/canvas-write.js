...
/**
 * What to send for a date the frontmatter carries — including when what it
 * carries is nothing.
 *
 * The dates were the one group of fields with no way back. Every other scalar
 * these strategies send gates on `!= null`, so writing `points_possible: 0` or
 * `published: false` reaches Canvas as itself; the dates gated on truthiness, so
 * a `due_at:` emptied out was indistinguishable from a `due_at` that had never
 * been written, and both left the key out of the request. Canvas keeps what it
 * is not sent, so the date stayed on the assignment forever and the markdown
 * quietly disagreed with it from then on.
 *
 * The distinction that fixes it has to come from the frontmatter, because
 * nothing else holds it. A base row stores hashes, not values — `local_hash` is
 * a sha256 of the whole file — so the engine can see that a file changed but
 * never that the change was a `due_at` going away. So an *absent* key still
 * means "Canvas keeps what it has", which is the only reading that is safe for
 * the courses whose frontmatter has never mentioned a date, and an explicit
 * `due_at:` with nothing after it is the instruction to clear.
 *
 * All three spellings of empty are that instruction — `due_at:`, `due_at: null`
 * and `due_at: ""` — because a YAML author will not think of them as different
 * things and Canvas takes `null` for all of them.
 *
 * A value that is really there passes through untouched, Date object included:
 * an unquoted ISO timestamp parses as one, and `JSON.stringify` renders it in
 * the shape Canvas wants.
 *
 * The clear does not survive a pull, and should not. Canvas answers with
 * `due_at: null`, `buildFrontmatter` in `lib/convert/html-to-markdown.js` writes
 * no key for that, and `due_at` is in `CANVAS_OWNED_KEYS` so the emptied key is
 * not carried over either. What is left is a file with no `due_at` and a Canvas
 * assignment with no due date — the two sides agreeing, which is the end state
 * the instruction was asking for. It is an imperative, and it is spent once it
 * has been obeyed.
 *
 * @param {*} value - The frontmatter value, or `undefined` when there is no key.
 * @returns {*} `undefined` to leave the field out of the request entirely.
 */
function dateToSend(value) {
...
const assignmentStrategy = {
  ...
  buildOpts: (title, html, frontmatter) => {
    ...
    if (frontmatter.due_at !== undefined)
      opts.dueAt = dateToSend(frontmatter.due_at);
    if (frontmatter.unlock_at !== undefined)
      opts.unlockAt = dateToSend(frontmatter.unlock_at);
    if (frontmatter.lock_at !== undefined)
      opts.lockAt = dateToSend(frontmatter.lock_at);
...
const discussionStrategy = {
  ...
  buildOpts: (title, html, frontmatter) => {
    ...
    if (frontmatter.delayed_post_at !== undefined)
      opts.delayedPostAt = dateToSend(frontmatter.delayed_post_at);
    if (frontmatter.lock_at !== undefined)
      opts.lockAt = dateToSend(frontmatter.lock_at);
    if (frontmatter.published != null) opts.published = frontmatter.published;
    return opts;
...
async function pushQuiz(
  ...
  if (
    ...
    if (matches.length > 1) {
      // Not "write the id into .canvas-sync.json", which is what this said
      // until the reconcile engine made it false. A row is matched to Canvas
      // through `matchBaseToCanvas`, which only ever looks at module items, so
      // a row naming a quiz that is not in a module yet matches nothing: the
      // planner reads the item as one whose Canvas copy is gone and writes
      // nothing at all. Putting the quiz in the module is what makes the row
      // reachable, and once it is there the row is not needed either.
      throw new Error(
        `"${title}" was not placed: ${matches.length} quizzes in this course ` +
          ...
          'Delete the stale one in Canvas, or add the one you mean to this ' +
          'module in Canvas by hand: a quiz already sitting in its module is ' +
          'claimed there by title, and this course-wide search never runs.',
      );
    }
