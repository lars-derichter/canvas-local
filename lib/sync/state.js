...
// What a Canvas address looks like is the Canvas layer's to say, and this
// module sits above it. Borrowed rather than redefined here so the shape
// `.env` and this file are compared in has exactly one definition.
const { normaliseBaseUrl } = require('../canvas/client');
 ...
 * between schemas, because two schemas that key items differently make a wrong
 * guess push duplicates to Canvas. `reset-sync-state` followed by a push is the
 ...
 * a real one.
 */
const RENAME_TEMP_KEY = '__cw_rename_';
