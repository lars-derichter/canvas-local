  ...
  if (fs.existsSync(file)) {
    ...
    if (data) {
      if (data.schema_version !== SCHEMA_VERSION) {
        // The other deliberate stop in this file, and the same kind: the author
        // is told which schema they have and the one command that upgrades it.
        throw new RefusalError(
 ...
 *
 * **Only the executor may call this**, and only where the Canvas module is
 * already gone or goes in the same action: `drop-base-module`,
 * `delete-canvas-module` and `delete-local-module` in `lib/sync/apply.js`. The
 * module row is what tells a later run "the author deleted this" from "this
 * tool has never seen it" — drop it while the Canvas module is still there and
 * `planModuleMetadata` emits `create-local-module`, writing the folder back to
 * disk, while `planModuleOrphan` returns before it can offer the module to
 * `--prune-canvas`. `cli/delete-module.js` used to call this and clear the
 * matching `state.files` rows on the way past, and did exactly that damage.
 *
