...
/**
 * Schema v4: the repo-relative path is the key and the Canvas ids are values —
 * the exact inverse of v3, which keyed every item by its Canvas identity
 ...
 *       "items": {
 *         "01-introduction/01-welcome.md": {        // repo-relative, POSIX
 *           "canvas_type": "page",
 *           "canvas_id": 1234,
 *           "page_url": "welcome",
 *           "module_item_id": 5678,
 ...
 * `synced_at`) are written by the sync engine. This module never computes one;
 * it only carries them through a rename or a move untouched.
 */

/**
 * A repo-relative path in the one shape this file stores it: forward slashes,
 ...
 * something to do quietly. Now that the file is committed the mismatch has a
 * second way in besides a re-`init`: a clone, or a branch, can arrive already
 * pointing at someone else's course.
 *
 ...
 * the file is missing or corrupt. Pass `{ allowNull: true }` to get null instead
 * of that default, which is how a command tells a first run from a synced course.
 *
 * A file written by another schema is refused rather than guessed at, and there
 * is deliberately no migration from v3: the two schemas key items differently,
 * so a wrong guess would push duplicates to Canvas. `reset-sync-state` followed
 * by a push is the upgrade path. A state describing a different Canvas course is
 * refused too — see `assertStateMatchesEnv` — except under
 * `{ skipEnvCheck: true }`, which `init` passes because it is the command that
...
/**
 * Find a module by the Canvas module id it carries.
 *
 * The v4 answer to v3's `findModuleEntryByFolder`, inverted along with the
 * schema: the folder is now the key, so it is the Canvas id that has to be
 * searched for. Pull needs it — Canvas hands it a module id and it has to work
 * out which local folder that is.
 *
 * @returns {[string, object]|null} [folder, entry], or null.
 */
function findModuleByCanvasId(state, canvasModuleId) {
  if (canvasModuleId == null) return null;
  const wanted = String(canvasModuleId);
  for (const [folder, entry] of Object.entries(state.modules || {})) {
    if (entry && entry.canvas_module_id != null) {
      if (String(entry.canvas_module_id) === wanted) return [folder, entry];
    }
  }
  return null;
}
 ...
 * silently would strand them, unreachable to any later prune. `delete-module`
 * clears them itself, behind its confirmation prompt, once the author has said
 * the Canvas objects should go too.
 *
 ...
 *
 * `state.files` is keyed by repo-relative path too, and its rows for embedded
