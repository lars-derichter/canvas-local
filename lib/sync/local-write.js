...
const { downloadFile, filePreviewPath } = require('../canvas/files');
const { hashBinaryFile } = require('./fingerprint');
...
/**
 * The title paragraph of a GFM alert, which is where its icon lives.
 *
 * `markdownToHtml` renders every alert with the Canvas-hosted icon inside
 * `<p class="markdown-alert-title">`, and `htmlToMarkdown` drops that paragraph
 * whole on the way back, so no markdown this tool writes ever names an icon. An
 * icon downloaded into `_files/` is therefore an orphan the moment it lands:
 * nothing references it, no `state.files` row records it, and the scanner never
 * looks inside `_files/` to find it again.
 */
const ALERT_TITLE_PATTERN =
  /<p\b[^>]*class=["'][^"']*markdown-alert-title[^"']*["'][^>]*>[\s\S]*?<\/p>/gi;
...
async function downloadReferencedFiles(
  ...
  let match;
  // Cut the alert titles out before the scan, so an alert icon is never a
  // candidate in the first place. Excluding it by **shape** is what makes the
  // exclusion hold: the id filter below knows only the six icons this state
  // file already names, which is none of them on a first pull into an empty
  // state, none of an earlier upload generation still embedded in a page that
  // has not been pushed since, and never the ones a page copied from another
  // Canvas course embeds from that course's own `/course-icons` folder. Each of
  // those downloaded a set of icons into the module's `_files/` and left them
  // there for good.
  const scannable = html.replace(ALERT_TITLE_PATTERN, '');
  while ((match = filePattern.exec(scannable)) !== null) {
  ...
  // Second line of defence, for an icon a hand edit in Canvas moved out of the
  // title paragraph: the ids this course's own icons are uploaded under.
  if (syncData.icons) {
  ...
  for (const [fileId, urlCourseId] of fileCourses) {
    const canvasUrlPattern = filePreviewPath(courseId, fileId);
    ...
    try {
      ...
      syncData.files[localRelPath] = {
        ...
        // The same function `apply` hashes an upload with, which is the whole
        // point: the two answers are compared to each other.
        sha256: hashBinaryFile(destPath),
      };
      canvasToLocal.set(canvasUrlPattern, localRelPath);
    } catch (err) {
...
function createPullFileResolver(courseId, currentFilePath, canvasToLocal) {
  return (href) => {
    ...
    const pattern = filePreviewPath(courseId, fId);
