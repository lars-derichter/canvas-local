 ...
 * never by guess.
 *
 * The sync state keys every item by its repo-relative path, so a file that
...
/**
 * The directory part of a repo-relative POSIX path; `''` for a bare filename.
 *
...
function detectRenames({ base = [], local = [], canvas = [] } = {}) {
  ...
  const placeKey = (itemPath, title) => `${directoryOf(itemPath)} ${title}`;
