...
const ORDER_POLICIES = new Set(['local', 'canvas', 'ask']);
...
function planCanvasUpdate(ctx, mc, itemPath, row, localItem, canvasItem) {
  ...
  if (localItem.canvasType && localItem.canvasType !== row.canvas_type) {
    ctx.report.skipped.push({
      kind: 'item',
      reason: 'type-changed',
      moduleFolder: mc.folder,
      itemPath,
      action: 'update-canvas-item',
      remedy:
...
function planOrdering(ctx, mc, unrecognised) {
  if (!mc.baseModule || !mc.localModule || !mc.canvasModule) return;
  ...
  if (!localMoved && !canvasMoved) return;
...
function planModuleMetadata(ctx, mc) {
  ...
  if (!baseModule) {
    ...
    // Paired by name with nothing in the state to link them: the module is
    // adopted rather than duplicated, and the local name is what it takes.
    if (
      localModule &&
      canvasModule &&
 ...
 *   pruneCanvas, pruneLocal, modules, resolved }`. `push` is
 *   `write: {canvas: true, local: false}` with `conflict: 'local'`, `pull` the
 *   mirror image, `status` writes to neither.
 * @returns {object} `{ actions, conflicts, skipped, orphans, decisions,
 *   unrecognised, ordering, pending, collision, withheld }`.
 */
