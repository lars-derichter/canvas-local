 ...
 *
 * `untimedReason` is for a caller `newest` cannot time at all, as opposed to one
 * whose timestamp Canvas failed to supply. The outcome is the same local win;
 * the explanation is not, and a caller with nothing to compare must not tell the
 * author their Canvas course returned bad data.
 *
 * @param {string} [untimedReason] Replaces the missing-`updated_at` reason, for
 *   a conflict over something neither side timestamps.
 ...
 *   reading its missing items as deleted.
