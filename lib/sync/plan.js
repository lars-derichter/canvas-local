 ...
 *
 * **A write the policy already forbids is not guarded, it is withheld.** Under
 * `push` every local write is suppressed by `emit` regardless, so recording a
 * skip for one would put a file the run was never going to touch under
 * "Skipped", with a remedy telling the author to commit or stash — and skips
 * fail the run. `withheld` is the truthful place for it, and that is where
 * `emit` puts it once this declines to intervene.
 */
function guardDirty(ctx, localItem, action, what) {
  if (!localItem || !localItem.dirty) return false;
  const side = ACTION_SIDES[action.type];
  if (side && ctx.policy.write[side] !== true) return false;
