 ...
 * carrying no `title:` — the last of which is every item this tool *adopted*
 * rather than created, because `writeTitleIfAbsent` runs on the create handler
 * alone.
 *
...
function planKnownItem(ctx, mc, entry, localItem, canvasItem, positions) {
  ...
  if (localItem && canvasItem) {
    ...
    if (!localChanged && !canvasChanged) {
      // Neither side's content moved — and this is the one branch the two local
      // changes no fingerprint covers can hide in. Every other branch below
      // already plans a write that carries both with it, so this is the whole
      // of the hole. One update settles either or both: the title rides on the
      // module item, and the push re-uploads whatever the body embeds.
      if (
