...
function planKnownItem(ctx, mc, entry, localItem, canvasItem, positions) {
  ...
  if (localItem && canvasItem) {
    ...
    if (!localChanged && !canvasChanged) {
      // Neither side's content moved — and this is the one branch a title
      // change can hide in. Every other branch below already plans a write that
      // carries the title with it, so this is the whole of the hole.
      if (hasTitleChanged(row, localItem)) {
