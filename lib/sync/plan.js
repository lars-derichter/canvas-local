...
function planModule(ctx, mc) {
  ...
  //
  // `writeLands` for the same reason every other refusal here uses it: under
  // `push` and `status` no local write is emitted at all, so nothing can throw
  // and nothing can land in the wrong place. Those runs keep the withheld
  // entries they already had.
  if (mc.folder === null && mc.canvasModule) {
    const wanted = mc.canvasModule.suggestedFolder;
    if (writeLands(ctx, 'create-local-module')) {
      ctx.report.skipped.push({
        kind: 'module',
        reason: 'folder-taken',
        moduleFolder: wanted,
