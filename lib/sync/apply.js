 ...
 *
 * The write goes through `writeMarkdown`, which is why this is async and why
 * every caller has to `await` it. Rewriting the frontmatter re-serialises the
 * whole file, so an unformatted round trip here would undo the formatting the
 * content write had just applied — on the one file the run had already got
 * right — and leave it reading as changed locally for ever.
 *
 ...
 * than none.
 *
 * `exceptFileRef` takes one `state.files` row out of the answer, and only
 * `deleteCanvasFile` passes it: there, the row named is the very thing being
 * asked about, so counting it would have every sweep answer "yes, itself".
 * Every other caller asks about a file no row of its own names, and passes
 * nothing.
 *
 * @returns {string|null} The first holder found, phrased for a log line.
 */
function fileStillReferenced(ctx, fileId, { exceptFileRef = null } = {}) {
    ...
    if (canvasType === 'file') {
      ...
      if (body) {
        await downloadReferencedFiles(
          ...
          maps.canvasToLocal,
          ctx.courseDir,
          { gitDirty: ctx.gitDirty, skipped: ctx.skipped },
        );
        ctx.invalidate();
      }
...
async function applyPlan(plan, options = {}) {
  ...
  for (const action of plan.actions || []) {
    ...
    try {
      ...
      if (outcome !== false) ctx.applied.push(action);
    } catch (err) {
