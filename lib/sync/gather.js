 ...
 * else.
 *
 * A non-markdown item is a binary the author dropped into a module, and
 * `hashLocalFile` would run its bytes through line-ending normalisation — which
...
function localFileHash(absolutePath, itemPath) {
  return itemPath.endsWith('.md')
    ? hashLocalFile(absolutePath)
    : hashBinaryFile(absolutePath);
}
 ...
 * @returns {string[]} One line per file recovered, and per file that could not be.
 */
...
function gatherLocal({ courseDir, gitDirty = null } = {}) {
  ...
  if (!fs.existsSync(courseDir)) {
    return { modules: [], warnings: [`${courseDir} does not exist.`] };
  }
  ...
  warnings.push(...recoverOrderTemps(courseDir));
