...
const { scanCourse } = require('../convert/course-scanner');
...
/**
 * The types with nothing behind the module item, so the item is the whole
 * fingerprint and no request can add to it.
 */
 ...
 * @returns {{available: boolean, paths: Set<string>, reason: string|null}}
 *   `paths` holds every dirty path relative to `courseDir`, POSIX, plus every
 *   directory that contains one — so a subfolder that became a text header can
...
function gitDirtyPaths({ courseDir, cwd = null, run = runGit } = {}) {
  ...
  try {
    ...
    return unavailable(`git status failed (${firstLine(err.message)})`);
  }
  ...
  return { available: true, paths, reason: null };
}
 ...
 */
const ORDER_TEMP_PREFIX = '__ccb_order_';
 ...
 * crash — leaves files sitting under `__ccb_order_*`. `scanCourse` skips every
 ...
 *   left under `__ccb_order_*` is invisible to `scanCourse`, so whatever that
...
/**
 * Every binary the whole course embeds, and whether that answer is whole.
 *
 ...
 * `__ccb_order_*` and therefore invisible to the scan all make it false. Every
 ...
 *   What `gitDirtyPaths` returned. Absent, or unavailable, marks every item
 *   dirty, which refuses every local write.
 * @returns {{modules: object[], warnings: string[],
 *   embedded: {refs: Set<string>, complete: boolean}}} `embedded` is
 *   `createReferenceSet` above: every binary the tree embeds, and whether the
 *   scan that found them saw all of it.
 */
function gatherLocal({ courseDir, gitDirty = null } = {}) {
  ...
  // Before the scan, never after: a file still parked under `__ccb_order_*` is
...
async function gatherCanvas({ courseId, base = null } = {}) {
  ...
  for (const mod of rawModules) {
    ...
    try {
      ...
      // A module whose items could not be read is reported as empty, which
      // would read as "everything in it was deleted on Canvas". Say so loudly
      // and hand back no items rather than a wrong answer dressed as a right
      // one; the planner's orphan rows are not deleted without `--prune`.
      warnings.push(
