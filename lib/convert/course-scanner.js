 ...
 *    `_category_.json`) and are skipped, so none is ever synced to Canvas.
 *    Names prefixed with `.` (`.DS_Store`) are skipped the same way. See
 *    isInternal.
 *  - Every item's `relativePath` is its path under course/ with forward
 *    slashes, on every platform. See buildFileItem.
 *
...
function scanCourse(coursePath) {
  ...
  for (const entry of entries) {
    // Only directories at the top level are modules; internal names are
    // skipped by the same rule as inside modules
 ...
 * Internal names are filtered out first, so `_files/` inside a subfolder
 ...
 *
 * `relativePath` is the item's path under course/ with forward slashes on every
 * platform. Two consumers take it as it comes and resolve against it with
 * `path.posix`: `cli/validate.js`, for the internal-link check and the
 * referenced-file check, and `cli/export.js`, through assemble and preprocess,
 * for image paths and cross-links. A native separator reaches both as a path
 * with no directory part, so they resolve from the course root instead of the
 * item's folder. Sync is not one of them, which is why it never saw this:
 * `lib/sync/gather.js` runs the value through `toPosixPath` at its own
 * boundary, before it becomes a `.canvas-sync.json` key or reaches the link
 * resolver.
 *
 * `path.posix.join` rather than `path.join(...).replace(/\\/g, '/')`: the parts
 * joined are directory entry names, and POSIX allows a backslash inside one, so
 * joining posix-wise leaves a file honestly named `a\b.md` intact where the
 * blind replace would split it into folders. That is a property of this
 * function, not a promise the pipeline keeps — `toPosixPath` is that same blind
 * replace, and downstream it is applied to this very value — but posix-joining
 * costs nothing and is strictly the better of the two here.
 */
function buildFileItem(fullPath, fileName, moduleFolder, subfolderName) {
  ...
  const relativePath = subfolderName
    ? path.posix.join(moduleFolder, subfolderName, fileName)
    : path.posix.join(moduleFolder, fileName);
...
/**
 * Whether a directory entry is internal to the tree rather than course
 * content. Two prefixes qualify. `_` is this tool's convention: `_files/`,
 * `_category_.json`, and anything an author wants kept out of every output.
 * `.` is the platform's convention for a hidden file, and it is here for
 * `.DS_Store` and its relatives (`._page.md` on an SMB share or a USB stick),
 * which Finder drops into any folder it has shown. `.gitignore` hides those
 * from git, not from a directory read: without this check a `.DS_Store` scans
 * as a `file` item, the planner sees a local-only item, and the next push
 * uploads it to Canvas. No course item can start with a dot under the naming
 * convention, so the skip is silent, like the underscore's.
 */
