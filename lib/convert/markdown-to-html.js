 ...
 *
 * Every value this file interpolates into an attribute goes through here, the
 * ones that cannot contain a special character included. The rule is worth more
 * than the bytes it saves: "did I escape this one?" is a judgement call that has
 * to be made again at every edit and was already answered wrong twice — a link
 * href and an image src went out raw for as long as the resolvers have existed,
 * so a file named `say"cheese.png` closed the attribute early and the pull read
 * the truncated URL back as the author's new intent.
 *
 ...
 * here — see decodeDestinationEntities below for marked's half of that
 * contract. Escaping alone would encode the `&` of an entity the author had
 * already written, which is how `&amp;` becomes `&amp;amp;`.
 *
...
const DESTINATION_ENTITY =
  /&(?:(amp|lt|gt|quot|apos)|#(\d{1,7})|#[xX]([0-9a-fA-F]{1,6}));/g;

/**
 * Decode the HTML entities in a link or image destination as CommonMark reads
 * them, so the escape that follows re-encodes rather than doubles up.
 *
 * This exists because of marked's contract: the renderer is handed the
 * destination as **raw source text**, entities undecoded, so the token for
 ...
 * what escapeHtmlAttr can produce, so escape ∘ decode is stable over its own
 ...
 *
 * The decode happens at the point of emission, on the pass-through path only.
 * A resolver is still handed, and still looks up, the raw source text: that is
 * the same string `extractFileReferences` (lib/convert/link-resolver.js) keys
 * the upload on, and the two have to agree on what a file is called. What a
 * resolver hands back is not source text at all — `canvasItemUrl` and the
 * `canvas_url` of a sync row assemble it out of ids from `.canvas-sync.json` —
 * so there is no encoding there to undo, and running a decode over it would be
 * a category error: it would rewrite a URL whose path genuinely contains the
 * characters `&amp;`, and a literal `&` in a future verifier URL needs escaping
 * rather than decoding anyway.
 *
 * Only the destinations. An image's alt text and a link's title reach the
 * renderer as raw source text too, and have been escaped without a decode since
 * that escaping was added, so `![a &amp; b]` still displays as `a &amp; b` in
 * Canvas. It is the same defect one field over, it round-trips stably, and it
 * is left as it is here rather than folded into a fix about attribute
 * injection.
 *
 * @param {string} str - A link or image destination, as marked hands it over.
 * @returns {string} The destination with those entities resolved.
 */
function decodeDestinationEntities(str) {
...
function markdownToHtml(markdownContent, options = {}) {
    ...
    extensions: [
      {
        ...
        renderer({ meta, tokens = [] }) {
          ...
          const cls = escapeHtmlAttr(`markdown-alert markdown-alert-${type}`);
          const color = escapeHtmlAttr(cfg.color);
          const background = escapeHtmlAttr(cfg.background);
          ...
          if (url) {
            ...
            imgHtml = `<img style="height: 0.8em; vertical-align: baseline;" src="${escapeHtmlAttr(url)}" alt="" /> `;
          }
  ...
  if (options.linkResolver || options.fileResolver) {
    ...
    rendererOverrides.link = function ({ href, title, tokens }) {
      ...
      if (!wasResolved) finalHref = decodeDestinationEntities(finalHref);
      const titleAttr = title ? ` title="${escapeHtmlAttr(title)}"` : '';
      const text = this.parser.parseInline(tokens);
      return `<a href="${escapeHtmlAttr(finalHref)}"${titleAttr}>${text}</a>`;
    };
    ...
    if (options.fileResolver) {
      rendererOverrides.image = function ({ href, title, text }) {
        ...
        if (!wasResolved) src = decodeDestinationEntities(src);
        // `title` and `text` get no decode: see decodeDestinationEntities.
        const titleAttr = title ? ` title="${escapeHtmlAttr(title)}"` : '';
        const alt = text ? escapeHtmlAttr(text) : '';
        return `<img src="${escapeHtmlAttr(src)}" alt="${alt}"${titleAttr}>`;
      };
    }
