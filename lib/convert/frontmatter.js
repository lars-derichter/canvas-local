...
/**
 * The YAML engine every write in this file goes through, and the reason it is
 * spelled out rather than left to gray-matter's own.
 *
 * **The dump moves to the top-level js-yaml; the parse deliberately does not.**
 * gray-matter bundles a js-yaml 3 of its own, and that dumper walks a string by
 * UTF-16 code unit. Each half of an emoji's surrogate pair fails its
 * `isPrintable` test, so a title carrying one is forced into double quotes and
 * written as `title: "\U0001F4D8 ..."` — read back correctly by every parser,
 * and unreadable to the author who opens the file. js-yaml 4 fixed it by
 * switching to `codePointAt`, and no option on js-yaml 3 turns it off.
 *
 * The parse stays gray-matter's, which is YAML **1.1**, because swapping it for
 * 1.2 would change how frontmatter already on disk reads. That is also why
 * `noCompatMode` is left at its default: it is what quotes `Yes`, `Off` and
 * `2024`, and without those quotes the 1.1 parser hands back a boolean or a
 * number instead of the string that went in. `renderFrontmatterKey` below is
 * built on that round-trip holding.
 *
 * `lineWidth: -1` never folds, so a long title stays on one line.
 */
const ENGINES = {
  yaml: {
    parse: matter.engines.yaml.parse,
    stringify: (data) => yaml.dump(data, { lineWidth: -1 }),
  },
};
...
function serializeFrontmatter(data, content) {
  ...
  return matter.stringify(content, data, { engines: ENGINES });
}
