...
function buildLinkMap(syncData) {
  ...
  for (const modData of Object.values(modules)) {
    ...
    // The path under `course/` is the key of a sync row, not a field on it.
...
/**
 * Run a regex over the code-masked view of the markdown and rebuild the real
 * string, applying `replacer` only to matches that fall outside code regions.
 *
 * Lives here rather than beside either caller because both of them need it and
 * they sit in different layers: the exporter rewrites image paths and cross
 * links with it (lib/export/preprocess.js), and the Canvas converter rewrites
 * the `[!ATTENTION]` marker with it (lib/convert/markdown-to-html.js). It is
 * `maskCodeRegions` with the answer put back, so it belongs next to it — and a
 * converter importing from the export layer to reach it would have been the
 * wrong way up.
 *
 * The mask decides what counts as code, so this inherits its reach exactly:
 * fenced blocks and inline spans are protected; an indented code block is not,
 * and neither is a fence nested inside a blockquote, whose backticks the
 * detector never sees behind the `>`. A caller's pattern also has to be written
 * knowing that a masked region keeps its newlines and loses only its
 * characters, so a `\s*` in it can still walk across one and match on the far
 * side.
 *
 * @param {string} md
 ...
 * @returns {string}
 */
function replaceOutsideCode(md, regex, replacer) {
  ...
  while ((m = regex.exec(masked)) !== null) {
    ...
    result += replacer(md.slice(start, end));
    last = end;
