...
/**
 * Extract `--ccb-*` custom properties from a CSS file.
 *
 * Comments are stripped first so a commented-out declaration does not count.
 * `var(--ccb-x)` references never match: a reference has no colon after the
...
/**
 * The theme's hex colours as pandoc variables, e.g. `ccb-alert-note-fg`.
