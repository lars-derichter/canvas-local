...
async function canvasRequest(method, path, body) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    if (attempt > 0) {
      ...
      console.log(`[canvas] Retrying ${method} ${path} (attempt ${attempt + 1}/${MAX_RETRIES + 1}) after ${delay}ms...`);
      await sleep(delay);
    }
    ...
    try {
      ...
      // Network error (ECONNRESET, timeout, etc.)
      lastError = err;
      if (attempt < MAX_RETRIES) continue;
      throw new Error(`Canvas API ${method} ${path} failed after ${MAX_RETRIES + 1} attempts: ${err.message}`);
    }
  ...
  if (Array.isArray(data)) {
    ...
    while (nextUrl) {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        ...
        try {
          nextResponse = await fetch(nextUrl, { method: "GET", headers });
        } catch (err) {
          if (attempt < MAX_RETRIES) continue;
          throw new Error(`Canvas API pagination GET ${nextUrl} failed after ${MAX_RETRIES + 1} attempts: ${err.message}`);
        }
        ...
        if (!nextResponse.ok) {
          if (isRetryable(nextResponse.status) && attempt < MAX_RETRIES) continue;
          let errBody;
          try {
