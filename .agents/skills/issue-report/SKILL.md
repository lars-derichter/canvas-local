---
name: issue-report
description: Quickly log an error or wanted change in course material into the issue queue at sources/issues.md: locates the file, quotes the passage to confirm, asks at most one clarifying question, and never diagnoses or fixes anything (that is /issue-fix). Use for "report issue", "log this for later", "found a mistake on page X", "issue melden", "noteer deze fout", "zet dit op de foutenlijst".
---
...
3. **Ask at most one clarifying question to pin the location**, only when the
   file or the passage stays ambiguous (several candidate files, no match for
   the quoted text, several matches). Present the candidates as options; if both
   file and passage are open, combine them in that one question. Still
   unresolved after the answer: log anyway with the best guess and a
   `location unverified` marker: `/issue-fix` sorts it out.
...
6. **Append the entry** at the end of `## Open`:

   ```
   - **YYYY-MM-DD · path** · [tag] · "exact quote" · what is wrong or
     wanted, one sentence · fix: … (optional)
   ```

   The `fix:` field carries the author's own proposal, verbatim, only when they
   stated one; never add your own. An issue spanning several files gets a glob
   or a short description in the path field. Create `sources/issues.md` (and
   `sources/` if needed) on first use, with this structure: an intro naming
   `/issue-report` and `/issue-fix` and welcoming hand-added bullets, a
   `## How to add an issue yourself` section with the bullet format and one
   example, `## Open`, and `## Resolved`. Quote the appended line back to the
   author as confirmation.
...
- Run `npm run format` on the queue file after appending; Prettier owns markdown
  wrapping.
