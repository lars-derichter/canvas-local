...
- **`/ai-tutor-build` turns any chatbot into a tutor for your course.** Telling
  ...
  this course covers or how it words things. The new skill writes one AI module
  under `course/` with a page per prompt type, from a guardrailed tutor and a
  concept explainer to a trace trainer, an exam coach and a teach-back checker,
  each carrying one prompt a student pastes into a new chat as the first
  message. Study packs go with them: markdown exports of your own material,
  generated by `npx course export -f md` and shipped as file items, which the
  student attaches so the chatbot answers from the course rather than from its
  training data. Two calls stay yours. Whether students may upload the material
  to a third-party service is asked before a single pack is generated, and
  whether a pack is still current is on you, because a pack is a snapshot of the
  day it was made; the report prints the regenerate command for each one. See
  [Students and AI](docs/ai-assistants.md#students-and-ai).
- **`/ai-policy-build` writes the course's AI-use page.** `/ai-tutor-build`
  leaves a stub where the rules belong, and the rules are the hard part: where
  help ends and handing in someone else's work begins differs per assessment
  moment, per course and per teacher. The skill interviews you moment by moment
  from one of three starting points, the AI Assessment Scale (AIAS), the
  two-lane approach of Liu and Bridgeman, or a custom policy from a deeper round
  of questions when neither framework fits, then plays every rule back as one
  short student-facing sentence for you to correct before anything is written.
  The finished page replaces the stub in place, so the links from the prompt
  pages keep working. It never invents institution policy: with none to name, it
  leaves a visible `TODO` where the link goes. Afterwards it offers
  `/course-context-update`, so the decisions land under Assessment in
  `context/course-context.md` instead of only on the page. See
  [Students and AI](docs/ai-assistants.md#students-and-ai).
