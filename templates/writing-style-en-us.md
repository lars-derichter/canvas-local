...
Page titles may start with an emoji signaling the page type. One is the default;
a second is fine when it genuinely adds information (❗️📅 for an assignment with
a deadline), never a third. With two, keep the order of the list below. If no
listed emoji fits, use an off-list one now and then; once it recurs, add it here
with its meaning.
